 ...
 * @param {Object} env - Environment variables from Vercel
 * @returns {Object} Path mappings configuration
 */
function getPathMappings() {
	try {
		// Check if DOMAIN_MAPPINGS is defined in the process.env object
		if (process.env.DOMAIN_MAPPINGS) {
			// If it's a string, try to parse it as JSON
			if (typeof process.env.DOMAIN_MAPPINGS === 'string') {
				return JSON.parse(process.env.DOMAIN_MAPPINGS);
			}
			// If it's already an object, use it directly
			return process.env.DOMAIN_MAPPINGS;
		}
	} catch (error) {
